...
export default async function ActivitiesPage() {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8"><Link href="/" className="text-sm text-teal-700 hover:underline">← Dashboard</Link><header className="my-7"><h1 className="text-3xl font-semibold tracking-tight">Activities</h1><p className="mt-2 text-sm text-slate-500">Your latest activities, newest first.</p></header>{activities.length === 0 ? <div className="panel p-6"><h2 className="font-semibold">No activities yet</h2><p className="mt-2 text-sm text-slate-500">Activities will appear here when they are available.</p></div> : <ul className="space-y-3">{activities.map((activity, index) => {
    ...
    return <li key={activity.id}><Link href={`/activities/${activity.id}`} className="panel flex flex-wrap items-center justify-between gap-4 p-5 transition hover:border-teal-300"><div><h2 className="font-semibold">{activity.name}</h2><p className="mt-1 text-xs text-slate-500"><span className="capitalize">{activity.type}</span> · {sourceLabels[activity.source]} · {(activity.distanceMeters / 1000).toFixed(2)} km</p></div><div className="flex items-center gap-4"><span className={`rounded px-2 py-1 text-[10px] font-semibold ${badge}`}>{verification.status}</span><span className="text-sm font-semibold text-teal-700">{reward} BKNE</span><span aria-hidden="true">→</span></div></Link></li>;
  })}</ul>}</main>;
}
