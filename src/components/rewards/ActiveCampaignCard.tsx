...
export default async function ActiveCampaignCard() {
  ...
  return <div className="@container"><div className="grid auto-rows-fr gap-3 @min-[30rem]:grid-cols-2 @min-[48rem]:grid-cols-3">
    {active.map((campaign, index) => {
      ...
      return <article key={campaign.id} className="panel campaign-card flex min-w-0 flex-col p-4">
        <div className="flex items-center justify-between"><span className="campaign-icon"><Icon name="campaign" className="size-5"/></span><span className="self-start rounded-md bg-teal-50 px-2 py-1 text-[10px] font-semibold text-teal-700">Active</span></div>
        ...
        <dl className="mt-3 grid grid-cols-2 gap-3 text-xs">
          ...
          <div><dt className="text-slate-500">Remaining pool</dt><dd className="mt-1 break-words font-semibold reward-metric">{(total?.remainingPool ?? campaign.remaining_pool).toLocaleString("en-US")} {campaign.token}</dd></div>
          {total && <>
            <div className="reward-metric"><dt>Distributed</dt><dd className="mt-1">{total.distributed.toLocaleString("en-US")} {campaign.token}</dd></div>
        ...
        <div className="mt-auto pt-3">{total && total.rewardPool > 0 && <div className="mb-3"><span className="mb-1.5 block text-[10px] text-slate-500">Pool distributed</span><progress aria-label={`${campaign.name}: distributed reward pool`} value={total.distributed} max={total.rewardPool} /></div>}
